...
import { Server, Code, Database, Terminal, ChevronRight, FileJson, ArrowRight } from 'lucide-react';
        ...
        <div className="xl:col-span-2 space-y-8">
          ...
          <div className="glass-panel p-8 rounded-xl border border-slate-700">
            ...
            <p className="text-slate-400 mb-4 leading-relaxed">
              Azure Sentinel Scout is designed to work with any backend that implements the REST API contract defined below. 
              By default, the application runs in <strong>Mock Mode</strong>. To connect a real Python/Node.js backend:
            </p>
            <ol className="list-decimal list-inside text-slate-300 space-y-2 mb-6 font-mono text-sm bg-slate-900/50 p-4 rounded border border-slate-800">
              <li>Go to <span className="text-cyber-cyan">Settings</span> in the sidebar.</li>
              <li>Switch "Data Source Mode" to <span className="text-cyber-cyan">Real Backend</span>.</li>
              <li>Enter your API Base URL (e.g., <span className="text-yellow-500">http://localhost:5000/api</span>).</li>
              <li>Ensure your backend supports CORS for the frontend origin.</li>
            </ol>
          </div>

          <h3 className="text-2xl font-bold text-white mt-12 mb-6">Endpoints</h3>
            ...
            response={`[
  {
    ...
    "total_findings": 20
          ...
          <EndpointCard 
            ...
            params={[
                { name: "run_id", type: "string", desc: "Optional. Filter by specific scan ID." }
            ]}
            response={`[
  {
    "id": "f-101",
            ...
            <div className="glass-panel p-6 rounded-xl border border-slate-700 sticky top-6">
                ...
                <div className="space-y-4">
                    <div className="p-3 bg-slate-900 rounded border border-slate-800">
                        <div className="text-xs font-bold text-slate-500 uppercase mb-1">Severity Enum</div>
                        <div className="font-mono text-xs text-cyber-cyan">
                            "High" | "Medium" | "Low" | "Informational"
                        </div>
                    </div>
                    <div className="p-3 bg-slate-900 rounded border border-slate-800">
                        <div className="text-xs font-bold text-slate-500 uppercase mb-1">Status Enum</div>
...
def get_runs():
  ...
  return jsonify(data)`}
                    ...
                    <p className="text-xs text-slate-500 mt-2">
                        See <code className="text-slate-300">backend/server.py</code> for full implementation details.
                    </p>
                </div>
            </div>
        </div>
...
const EndpointCard: React.FC<{ method: string, path: string, desc: string, params?: any[], response: string }> = ({
    ...
    return (
        <div className="glass-panel rounded-xl border border-slate-800 overflow-hidden">
            <div className="p-6 border-b border-slate-800/50 bg-slate-900/50 flex items-start justify-between">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <span className={`px-2 py-1 rounded text-xs font-bold font-mono border ${methodColors[method]}`}>
                            {method}
                        </span>
            ...
            <div className="p-6 bg-slate-950/30">
                {params && (
                    <div className="mb-6">
                        <h5 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Query Parameters</h5>
                        <div className="bg-slate-900 rounded border border-slate-800 overflow-hidden">
                            <table className="w-full text-left text-xs">
                                <tbody className="divide-y divide-slate-800">
                                    {params.map((p, i) => (
                                        <tr key={i}>
                                            <td className="p-3 font-mono text-cyber-cyan w-32">{p.name}</td>
                                            <td className="p-3 font-mono text-yellow-500 w-24">{p.type}</td>
