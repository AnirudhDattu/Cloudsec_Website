...
const listFindingsTool: FunctionDeclaration = {
  name: "listFindings",
  description:
    "Retrieve security findings. Can filter by severity (High, Medium, Low) or Service name.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      severity: {
      ...
      service: {
        type: Type.STRING,
        description:
          "Filter by Azure Service name (e.g., Storage Accounts, SQL Database)",
      },
    },
  },
};
...
const getTrendTool: FunctionDeclaration = {
  name: "getTrend",
  description:
    "Get the historical vulnerability trend data (counts of High/Medium/Low over time).",
...
const triggerScanTool: FunctionDeclaration = {
  name: "triggerScan",
  description:
    "Initiate a new immediate security scan of the Azure infrastructure.",
  parameters: { type: Type.OBJECT, properties: {} },
};
  ...
  parameters: {
    ...
    properties: {
      page: {
        type: Type.STRING,
        description:
          "The page to navigate to. Options: 'dashboard', 'findings', 'reports', 'settings', 'home'.",
      },
    },
    required: ["page"],
  },
        ...
        case "listFindings":
          ...
          if (args.service) {
            filtered = filtered.filter((f) =>
              f.service.toLowerCase().includes(args.service.toLowerCase())
            );
          }
    ...
    try {
      ...
      while (result.functionCalls && result.functionCalls.length > 0) {
        ...
        for (const call of result.functionCalls) {
          ...
          // CRITICAL FIX: The Gemini API "functionResponse.response" field MUST be a JSON Object (Map).
          // It cannot be a primitive or an Array. If the tool returns an array (like listFindings), we must wrap it.
      ...
      let errorMessage =
        "Connection Error: Unable to communicate with Neural Core.";
      if (error.message) {
    ...
    <div className="h-[calc(100vh-6rem)] p-4 md:p-8 flex flex-col animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-slate-400 font-mono text-sm mt-1">
          MODEL: GEMINI-3-PRO-PREVIEW // STATUS: ONLINE
        </p>
      </div>
      ...
      <div className="flex-1 bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden flex flex-col backdrop-blur-sm shadow-2xl relative">
        ...
        <div className="p-4 bg-slate-900 border-t border-slate-800">
          ...
          <div className="max-w-4xl mx-auto mt-3 flex flex-wrap justify-center gap-3">
            <button
              onClick={() => setInput("Start a new security scan immediately.")}
              className="text-[10px] text-slate-500 hover:text-cyber-cyan border border-slate-800 hover:border-cyber-cyan/30 px-3 py-1.5 rounded-full transition-all flex items-center gap-1 uppercase tracking-wider font-bold"
            >
              <Activity size={10} /> Trigger Scan
            </button>
            <button
              onClick={() =>
                setInput("List High severity findings regarding SQL.")
              }
              className="text-[10px] text-slate-500 hover:text-cyber-cyan border border-slate-800 hover:border-cyber-cyan/30 px-3 py-1.5 rounded-full transition-all flex items-center gap-1 uppercase tracking-wider font-bold"
            >
            ...
            <button
              onClick={() => setInput("Take me to the dashboard.")}
              className="text-[10px] text-slate-500 hover:text-cyber-cyan border border-slate-800 hover:border-cyber-cyan/30 px-3 py-1.5 rounded-full transition-all flex items-center gap-1 uppercase tracking-wider font-bold"
            >
