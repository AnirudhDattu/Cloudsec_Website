...
const FindingsTable: React.FC = () => {
  ...
  const getSeverityBadge = (s: Severity) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded text-xs font-mono border ${colors[s]} uppercase tracking-wide`}
      >
        {s}
      </span>
    );
  ...
  if (loading)
    return (
      <div className="p-12 text-center font-mono text-cyber-cyan">
        FETCHING_DATA...
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 font-mono text-sm mt-1">
            {filteredFindings.length} ISSUES DETECTED
          </p>
        </div>
      ...
      <div className="flex-1 overflow-auto border border-slate-800 rounded-lg bg-slate-900/50 backdrop-blur-sm custom-scrollbar">
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-900 sticky top-0 z-10">
            <tr>
              <th className="p-4 text-xs font-bold text-slate-400 tracking-wider border-b border-slate-800">
                SEVERITY
              </th>
              <th className="p-4 text-xs font-bold text-slate-400 tracking-wider border-b border-slate-800">
                RULE ID
              ...
              filteredFindings.map((finding) => (
                <React.Fragment key={finding.id}>
                  ...
                  >
                    ...
                    <td className="p-4 font-mono text-cyber-cyan text-sm">
                      {finding.rule_id}
                    </td>
