...
const Dashboard: React.FC = () => {
  ...
  const trendFixed = trend.map((t) => ({
    ...t,
    date: new Date(`2024-${t.date.replace("/", "-")}`), // dummy year
  }));
...
const KpiCard: React.FC<{
  ...
  <div
    className={`relative overflow-hidden rounded-xl border ${borderColor} ${bg} p-6 transition-all hover:shadow-lg hover:shadow-black/20`}
  >
    <div className="flex justify-between items-start mb-4">
      <div>
        ...
        <h4 className={`text-4xl font-bold tracking-tight ${color}`}>
          {value}
        </h4>
      </div>
