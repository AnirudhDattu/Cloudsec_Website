...
import { Save, Server, Database, CheckCircle, XCircle, RefreshCw, Settings as SettingsIcon } from 'lucide-react';
  ...
  const handleTestConnection = async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            setStatus('ERROR');
            setStatusMsg(`Backend reachable but returned ${res.status} ${res.statusText}`);
        }
    } catch (error) {
        setStatus('ERROR');
        setStatusMsg('Failed to connect. Ensure backend is running and CORS is enabled.');
    }
  };
  ...
  return (
    <div className="p-6 md:p-12 h-full animate-fade-in">
        <header className="mb-12">
            ...
            <p className="text-slate-400 font-mono text-sm mt-1">MANAGE DATA SOURCE AND API CONNECTIONS</p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="glass-panel p-8 rounded-xl border border-slate-700">
                ...
                <div className="flex gap-4 mb-6">
                    <button 
                        ...
                        className={`flex-1 py-4 rounded border transition-all ${useMock ? 'bg-cyber-cyan/20 border-cyber-cyan text-cyber-cyan' : 'bg-slate-900 border-slate-700 text-slate-500 hover:bg-slate-800'}`}
                    >
                    ...
                    >
                        ...
                        <div className="text-xs opacity-70">Connect to local Python API</div>
                    </button>
                </div>
            ...
            <div className={`glass-panel p-8 rounded-xl border border-slate-700 transition-opacity ${useMock ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                    <Server size={20} /> API CONNECTION
                </h3>

                <div className="mb-6">
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">API Base URL</label>
                    <input 
                        type="text" 
                        value={apiUrl}
                ...
                <div className="flex items-center justify-between">
                    ...
                    >
                        {status === 'TESTING' ? <RefreshCw className="animate-spin" size={16} /> : <RefreshCw size={16} />}
                        TEST CONNECTION
                    </button>
        ...
        <div className="mt-12 border-t border-slate-800 pt-8">
            ...
            <p className="text-slate-400 text-sm mb-4">
                To connect your own backend, ensure your API matches the JSON structure defined in the app.
            </p>
            <div className="bg-slate-900 p-4 rounded border border-slate-800 font-mono text-xs text-slate-300">
                See <strong>API_CONTRACT.md</strong> in the project root for the full specification.
            </div>
        </div>
    </div>
  );
